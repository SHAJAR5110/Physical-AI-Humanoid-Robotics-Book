...
class RAGRetriever:
    ...
    def query_qdrant(self, query_embedding: List[float], top_k: int = 5, threshold: float = 0.0) -> List[Dict]:
        ...
        """
        try:
            # Perform similarity search in Qdrant using the correct API
            # Note: Using search() method with proper parameter names
            search_results = self.qdrant_client.search(
                ...
                with_vectors=False  # Don't return vectors, just metadata
            )
            ...
            for result in search_results:
                ...
                formatted_result = {
                    ...
                    "position": payload.get("position", 0) if payload else 0,
                    "similarity_score": result.score if hasattr(result, 'score') else 0.0,
                    "chunk_id": result.id if hasattr(result, 'id') else None,
                    "created_at": payload.get("created_at", "") if payload else ""
                }
                formatted_results.append(formatted_result)

            logger.info(f"Formatted {len(formatted_results)} results from Qdrant")
            return formatted_results

        except AttributeError as e:
            logger.error(f"AttributeError querying Qdrant (API mismatch): {e}")
            logger.error(f"Available methods: {[m for m in dir(self.qdrant_client) if not m.startswith('_')]}")
            return []
        except Exception as e:
